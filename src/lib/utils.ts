...
export function errorMessage(error: unknown) {
  ...
  if (message.includes('Email not confirmed'))
    return 'Confirme seu e-mail antes de entrar. Confira também a caixa de spam.'
  ...
  if (message.includes('Failed to fetch') || message.includes('NetworkError'))
    return 'Sem conexão no momento. Confira sua internet e tente novamente.'
...
export function distanceKm(
  lat: number,
  lng: number,
  r: Pick<Restaurant, 'latitude' | 'longitude'>,
) {
  const rad = Math.PI / 180
  const a =
    Math.sin(((r.latitude - lat) * rad) / 2) ** 2 +
    Math.cos(lat * rad) *
      Math.cos(r.latitude * rad) *
      Math.sin(((r.longitude - lng) * rad) / 2) ** 2
  return 6371 * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a))
}
export const distanceText = (km: number) =>
...
export function splitBill(data: BillData) {
  ...
  for (const item of data.items) {
    ...
    consumers.forEach((p, i) => {
      totals[p] += base + (i < amount % consumers.length ? 1 : 0)
    })
  }
  ...
  return {
    ...
    unassigned: unassigned + service - assignedService,
  }
}
...
export function parseReceipt(text: string) {
  return text.split('\n').flatMap((line) => {
    ...
    if (
      !match ||
      /total|troco|dinheiro|cart[aã]o|cnpj|tribut|servi[cç]o|desconto|subtotal/i.test(match[1])
    )
      return []
    return [
      {
